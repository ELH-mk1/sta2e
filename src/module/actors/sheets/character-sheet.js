...
export class STACharacterSheet extends ActorSheet {
  ...
  get template() {
    ...
    if (!foundry.utils.isNewerVersion(versionInfo,"0.8.-1")) return "systems/sta2e/templates/actors/character-sheet-legacy.html";
    return `systems/sta2e/templates/actors/character-sheet.html`;
  }
  ...
  getData() {
    ...
    let maxAttribute = 12;
    const overrideAttributeLimitSetting = game.settings.get('sta2e', 'characterAttributeLimitIgnore');
    ...
    let minDiscipline = 0;
    let maxDiscipline = 5;
    const overrideDisciplineLimitSetting = game.settings.get('sta2e', 'characterDisciplineLimitIgnore');
    ...
    //Stress Mod check
    if (!(sheetData.system.stmod))
      sheetData.system.stmod = 0;
  ...
  activateListeners(html) {
    ...
    function stressTrackUpdate() {
      ...
      // Manual Adjustment
      stressTrackMax += parseInt(html.find('#stmod')[0].value)
    ...
    const repPointsMax = game.settings.get('sta2e', 'maxNumberOfReputation');
