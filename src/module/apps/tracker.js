export class STATracker extends Application {
  ...
  static get defaultOptions() {
    ...
    options.template = 'systems/sta2e/templates/apps/tracker.html';
  ...
  static LimitOf(resource) {
    return resource == STATracker.Resource.Momentum ? game.settings.get('sta2e', 'maxNumberOfMomentum') : 99999999;
  }
  ...
  static ValueOf(resource) {
    return game.settings.get("sta2e", resource);
  }
  ...
  static UserHasPermissionFor(resource) {
    let requiredLevel = game.settings.get("sta2e", `${resource}PermissionLevel`);
  ...
  static async DoUpdateResource(resource, value) {
    ...
    if(STATracker.UserCanWriteSettings()) {
      await game.settings.set('sta2e', resource, value);
  ...
  static async OnSocketData(message) {
    switch (message.type) {
      case STATracker.MessageType.SetResource:
        if (STATracker.UserCanWriteSettings()) {
          await game.settings.set('sta2e', message.resource, message.value);
          STATracker.SendUpdateMessage(STATracker.MessageType.UpdateResource);
          STATracker.UpdateTracker();
        }
        break;
